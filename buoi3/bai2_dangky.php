<?php
	...
	try{
		...
		echo '<div class="error-box">Bạn đã đăng nhập rồi không thể đăng ký <a href="bai2_thongtincanhan.php">Xem thông tin cá nhân</a></div>';
		$mcon->close();
		...
		try{
			if(isset($_POST['register'])){
				...
				if($s_userdata_error['username'] != '' || $s_userdata_error['password'] != '' || $s_userdata_error['avatar'] != '' || $s_userdata_error['gender'] != '' || $s_userdata_error['major'] != '' || !$ok_hobby){
					...
					echo '<div class="success-box">Đăng ký thành công tài khoản '.$username.' <a href="bai2_dangnhap.php">đăng nhập</a></div>';
					$success = true;
				}
				$mcon->close();
			}
